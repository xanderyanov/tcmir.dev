...
gulp.task("image", function() {
  return gulp
    .src("assets/img/**/*.*") //Выберем наши картинки
    .pipe(
      imagemin({
        //Сожмем их
        progressive: true,
...
gulp.task("myJs", function() {
  return gulp
    .src("assets/js/**/*.*") //Выберем наши картинки
    .pipe(plumber())
    .pipe(sourcemaps.init()) //Инициализируем sourcemap
    .pipe(uglify()) //Сожмем наш js
    .pipe(sourcemaps.write("."))
...
gulp.task(
  ...
  gulp.series(
    "clean",
    gulp.parallel("styles", "html", "image", "js", "myJs", "fonts")
  )
);

gulp.task("watch", function() {
  gulp
    .watch("assets/css/**/*.*", gulp.series("styles"))
    .on("uplink", function(filepath) {
      remember.forget("styles", path.resolve(filepath));
      delete cached.caches.styles[path.resolve(filepath)];
    });
  gulp
    .watch("assets/img/**/*.*", gulp.series("image"))
    .on("uplink", function(filepath) {
      remember.forget("image", path.resolve(filepath));
      delete cached.caches.image[path.resolve(filepath)];
    });
  gulp
    .watch("assets/js/**/*.*", gulp.series("myJs"))
    .on("uplink", function(filepath) {
